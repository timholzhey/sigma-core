...
TEST_DEF(test_preprocessor, precedence_parentheses) {
	int num_tokens_in = 7;
	token_t tokens_in[num_tokens_in];
	tokens_in[0].type = TOKEN_TYPE_PAREN_OPEN;
	tokens_in[1].type = TOKEN_TYPE_VAR;
	tokens_in[2].type = TOKEN_TYPE_OPERATOR_ADD;
	tokens_in[3].type = TOKEN_TYPE_NUM;
	tokens_in[4].type = TOKEN_TYPE_PAREN_CLOSE;
	tokens_in[5].type = TOKEN_TYPE_OPERATOR_POW;
	tokens_in[6].type = TOKEN_TYPE_NUM;
	int num_tokens_out = 0;
	token_t tokens_out[1000];
	...
	token_type_t expect_token_types[] = {TOKEN_TYPE_PAREN_OPEN, TOKEN_TYPE_VAR, TOKEN_TYPE_OPERATOR_ADD, TOKEN_TYPE_NUM,
										 TOKEN_TYPE_PAREN_CLOSE, TOKEN_TYPE_OPERATOR_POW, TOKEN_TYPE_NUM};

	TEST_ASSERT_EQ(ret, RETVAL_OK);
	TEST_ASSERT_EQ(err, PREPROCESSOR_ERROR_OK);
	TEST_ASSERT_EQ(num_tokens_out, num_tokens_in);
