...
TEST_DEF(test_preprocessor, precedence_simple) {
	...
	retval_t ret = preprocessor_process(tokens_in, num_tokens_in, tokens_out, &num_tokens_out, 1000);
...
TEST_DEF(test_preprocessor, fail_insufficient_space) {
	...
	retval_t ret = preprocessor_process(tokens_in, num_tokens_in, tokens_out, &num_tokens_out, 1);
	preprocessor_error_t err = preprocessor_errno();
...
//	for (int i = 0; i < num_tokens_out; i++) {
//		printf("[%s] ", token_type_name_map[tokens_out[i].type]);
//	}
